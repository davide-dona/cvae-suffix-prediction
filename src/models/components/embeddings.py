...
class EventEmbeddings(nn.Module):
    ...
    def __init__(self, config: EmbeddingConfig, dataset_info: DatasetInfo):
        ...
        self.activity_embedding = nn.Embedding(
            dataset_info.num_activities, config.activity_dim, padding_idx=dataset_info.pad_activity_index
        )
        self.resource_embedding = nn.Embedding(
            dataset_info.num_resources, config.resource_dim, padding_idx=dataset_info.pad_resource_index
        )
    ...
    def forward(
        ...
        return torch.cat(
            (
                ...
                timestamps.unsqueeze(-1),             # [batch_size, seq_len, 1]
            ),
            dim=-1,
